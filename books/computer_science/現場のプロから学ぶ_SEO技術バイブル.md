...
これはかなり良い本だった。というのも、内容が具体的かつ実践的で、どこを読んでも無駄な部分がないと感じた。

特に、新しい書籍なためか、購入時点（2018/08頃）2018の話題がたくさん盛り込まれてて勉強になった。

自分の発見としては、Googleの検索の主軸がスマートフォンへシフトしてきていて、

サイトを運営する側もスマホを意識したサイト作りに力を入れないとだめな時代なんかーと感じた。

以下、本に書いてあった内容をまとめる（内容が良かったので大部分が本からの引用になってしまってるが、、、まあいいか）
...
Googleは検索クエリからユーザーのニーズを推測してるらしい。
で、この推測されたニーズにマッチしているかどうかの評価のことらしい
また、この推測に利用されるのはモバイルユーザーのみを対象にしてるみたいなので注意。

※モバイルでの検索が検索全体の過半数を占めているため


### 低いNeeds Met評価を受けるケース
...
> Googleのアルゴリズムに「QDD」は存在するのか？その意味や仕組みを解説 ｜ SEO World  
> https://seoworld.jp/qdd

また、実は未知の検索クエリが思った以上に多いっぽい

SMX West 2011で、GoogleはAdWordsのキーワードツールセッションで以下のデータを示したそう。

検索クエリの...

 - 44%は3ワード以上
 - 64%はAdWords広告で出向されたキーワードと完全一致しなかった
 - 20%は過去90日間存在しなかった

また、毎日15%の全く新しい検索クエリが生まれてるらしい。
...
検索エンジンは、入力されたクエリを以下の4分類で解釈するらしい。
 ...
 - **インフォメーショナルクエリ（別名：情報型、Know query）**
   - ユーザーの意図：「調べたい、情報を見つけたい」
   - 検索クエリ全体の約80%を占める
   - 潜在顧客に相当する
 - **トランザクショナルクエリ（別名：取引型、Do query）**
   - ユーザーの意図：「アクションを起こしたい（購入、入会、契約 ...等）」
   - コンバージョンに結びつきやすい顧客に相当
 - **ナビゲーショナルクエリ（別名：案内型、Website query）**
   - ユーザーの意図：「特定のページに行きたい」
   - Wiki、Amazon、Facebook、Twirtterなど
 - **ビジットインパーソンクエリ（別名：エリア型、Location query）**
   - ユーザーの意図：「自身の近くの情報を探したい」
   - 街で食べログを見てる的な
   - あるいは近くにどんな場所があるか調べてる的な

検索エンジンが検索クエリをどう解釈したかを判別する方法があるらしい。
...
以下のようになる
 ...
 - **インフォメーショナルクエリ／ビジットインパーソンクエリ**
   - 検索ボックス下部のメニュー：全て、地図、画像、ニュース、動画、もっと見る
   - メインカラムの検索結果：地図と場所が列挙される
   - 右カラム：基本情報が表示される（ラーメンについてなど）
 - **トランザクショナルクエリ**
   - 検索ボックス下部のメニュー：すべて、ショッピング、画像、地図、ニュース、もっと見る
...
ユーザーのニーズに合わせたコンテンツを費用対効果を最大にして作るためには
 ...
 - **検索ボリュームから「ニーズの大きさ」を知る**
  - 調査可能なツールがたくさんあるっぽい
 - **検索結果の上位に表示される競合サイトから「競合性」を知る**

が必要。その具体的なプロセスは書籍参照。

結構細かく具体的な手順がまとめられていた。参考になる。
...
**内部要素**
 ...
 - インデックス数
   - googleで`site:www.mydomain.com`で調べられる（件数=インデックス数）
 - サイト全体でのコンテンツの網羅性
   - ページ内容をそれぞれ比較（泥臭い作業）
 - ページ単体でのコンテンツの網羅性
   - ページ内容をそれぞれ比較（泥臭い作業）
   - メインコンテンツの文字数を単純比較するのも1つの指標になる
 - その他基本的な内部施策状況（htmlの基本的なSEO）
   - [HTMLの記述方法](#htmlの記述方法)の基本的な対応の実施状況
...
## 2つの外部要素
 ...
 - 被リンク
   - 明示的な引用
   - 外部サイトからのa要素でのリンク
   - ただし、`nofollow`が設定されている場合は評価されない
 - サイテーション
   - 暗示的な引用
   - a要素でないリンクでの言及
   - `nofollow`でのリンク（実は評価される可能性があるらしい）

### 被リンク

**リンク元のページ評価（ページランク）**が基礎で、それ以外の項目は＋アルファとして扱われる。

 - **リンク元のページ評価（ページランク）**
   - 被リンク数が多ければ価値が高くなる
   - 高品質なページからの被リンクは価値が高くなる
   - リーズナブルサーファーモデルというアルゴリズムで評価されてる
     - ユーザーにとってより価値が高い発リンクのページランクに重みを加えて分配するモデル
 - **リンク元ページにおけるリンクの掲載状況**
   - アンカーテキストのフォント、色、その他書式（太字、イタリック）
   - リンクの出現箇所（ファーストビュー、フッター、サイドバー、リストタグなど）
   - アンカーテキストの単語数
   - リンクの商用度
 - **リンク元とリンク先のコンテンツの関連性**
 - **リンクが張られてからの経過時間（リンクエイジ）**
   - リンクが初めて張られてからの経過時間
   - 新しいリンクが多いページを高く評価するらしい
   - 被リンクの新規獲得数・消滅数の時系列での前後比較
   - 全被リンクの内、直近n日間に新規獲得した被リンク数の比率
   - 時事的なトピック以外での急速な被リンク増加はスパム判定を受ける可能性がある

> Googleがリンクの価値を決定する34の要素 ｜ SEO Japan – SEO・CRO情報ブログ by IOIX  
> http://www.seojapan.com/blog/google%E3%81%8C%E3%83%AA%E3%83%B3%E3%82%AF%E3%81%AE%E4%BE%A1%E5%80%A4%E3%82%92%E6%B1%BA%E5%AE%9A%E3%81%99%E3%82%8B34%E3%81%AE%E8%A6%81%E7%B4%A0

### サイテーション

検索エンジンがサイテーションとして認識する要素はいかの3つ

 - ブランド名（会社名、店舗名、サービス名など）
 - 住所、電話番号
 - a要素ではないテキストで記載されたURL

### Webページとサイト情報の紐付け

検索エンジンに、Webページとサイト情報を紐付けて認識させるには以下の対応が必要
 ...
 - 企業や店舗など拠点がある場合の対応
   - Google枚ビジネスに登録する
   - その他情報メディアに掲載する
     - iタウンページ、Indeed、Wantedly、スタンバイなど

## 効果的なリンクビルディング

 - 自社コーポレートサイトからリンクを貼る（企業ならこれか）
 - サテライトサイトからリンクを貼る（企業ならこれか）
 - 他社サイトからのリンク（これむずい）

リンク作成のみを目的とするサテライトサイトはペナルティ対象になるので注意。

※サテライトサイト：自社サイトと関連するコンテンツを扱う別の自社サイトのこと
...
### 他サイトからのリンク獲得
 ...
 - **リンクベイト**
   ...
   - 以下がリンクベイトにおすすめな記事の例
     - まとめ記事（例：〜に関するXX個のまとめ）
     - 一次情報記事（例：2018年度の内定率調査結果）
     - 流行記事（例：速報！〜の謎解明）
     - ユーモア記事（例：笑ったら負け、爆笑動画）
     - 煽り記事（例：文字数が多いとSEOに逆効果？）
     - 攻撃的記事（例：巷で人気の〜を使ったら大失敗した件）
 - **リンケラティ**
   - リンクを張れるユーザー（ブロガー、Webニュースの記者、執筆者などのWebコンテンツの作成者のこと）
   - リンクベイトを見てもらってリンクを張ってもらう
   - SNSなどでの紹介を狙うのも効果的

## SNSでのリンクビルディング

SNSでのフォローやいいねなどはnofollow値が付与されているため、被リンク効果は得られないと明言されている。

ただし、この拡散によってリンケラリティからの被リンクを獲得できる機会が増えるため、結果的にリンクビルディングに有効となる。

### OGP設定による適切なページ情報の表示

 - Open graph protocol
 - htmlタグ内に情報を埋め込む（`<meta property="og:title" content="OG Title">`みたいな）
 - SNSで共有された際に意図するタイトル、URL、サムネイル等を表示させられる
...
このOGP設定も全ページでユニークであることが望ましいみたい。

FacebookやTwitterでは、このOGP設定を確認するツールが提供されているので利用しても良いかも。

### SNSボタンの設定

記事ページにSNSボタンを設置して、シェアしてもられるようにする。
こうすることで、リンケラリティからの被リンクを獲得できる機会が増える。
...
新・旧SearchConsoleやGoogleのAPI等でそれぞれデータを取得できるので定期的に収集しましょう。

### ドメイン・URL変更

基本的にはURL永続性を厳守した方が良い。

#### URL変更のNGパターン

もし変更することになっても、以下のようなNGパターンには注意すること。

 - リダイレクト元とリダイレクト先のページ内容が一致していない（全然違うページに飛んじゃう）
 - 全て一律で同じURLにリダイレクトする（全て新URLのトップページへ、など）
 - 複数回のリダイレクト

#### ドメイン変更の対応

リダイレクト処理、XMLサイトマップのURL変更に加えて、旧Search Consoleでアドレス変更のリクエストを送る。

### 特殊ケース対応

#### コンテンツシンジケーション

コンテンツシンジケーションとは、より多くのユーザーに拡張するために、Webサイトの記事をニュースメディアに提供すること。

この時、自サイトと同じ内容のコンテンツが複数サイトで表示されることになるため、重複コンテンツのペナルティを受ける可能性がある。

対策として、シンジケート先のメディアに出典元として自サイトのURLを明記してもらうようにする。
