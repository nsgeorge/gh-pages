...
このため、誰がいくらビットコインを持っているのかをノード全体で正確に認識する必要があります。
...
このため、この指定された公開鍵に対応する秘密鍵を持つ人だけがそのトランザクションのOutputをUnlockして使う事ができる、という事になります。この

> 各トランザクションのOutputは指定された公開鍵でLockされています

部分を上の図で説明すると、左のトランザクションの「Alice's public key」がOutputをLockしている部分に該当します。
このLockは、左のトランザクションの下にある「Alice's private key」でのみUnlockして利用することができます。
なので、Aliceは自身が持つ秘密鍵 = Alice's private keyでOutputをUnlockすることになります。
...
「[トランザクション](#トランザクション)」では、ビットコインにおけるトランザクションのデータ構造を軸に、
各ユーザーの取引について嘘の取引が行われない仕組みの概要を説明しました。
この仕組のおかげで、嘘の取引、不正な取引を防ぐことができています。
次に、こういったトランザクションが何千、何万とやりとりされる中で、
全てのノードが全ユーザーの残高についてどのように共通認識を得ているかについて説明します。

実は、トランザクションのOutputは、作られた直後からすぐ使えるというわけではありません。
そのトランザクションがビットコインネットワークのノード全体で「承認」されて、
初めてそのトランザクションのOutputを利用できるようになります。
この「承認」とは、全ノードが共有する「公開元帳」にそのトランザクションを正式に記録する作業を言います。
つまり、「このトランザクションは十分に検証され、正式な取引として認めらたので、皆が手元に持っている元帳に正式に記録することにします」という意味合いになります。
この皆が手元に持っている公開された元帳こそが「ブロックチェーン」です。  
ブロックチェーンには、過去に承認された全てのトランザクションが記録されており、
それらは過去に遡れば遡るほど信頼度の高いトランザクションとみなされます。
信頼度の高いトランザクションとはすなわち「改竄が難しい」「この先変更される可能性が限りなく少ない」という事を意味します。
ブロックチェーンは以下の図のようなデータ構造になっています。

---

![ブロックチェーンのデータ構造](../resources/img/bitcoin_blockchain_simple.svg)

*ブロックチェーンのデータ構造*

---

ブロックチェーンは「ブロック」というデータ構造が順々に連なった構造になっています。
ブロックは、上の図でいう「hash」を持つ箱の単位で表現されるデータ構造です。
このhashは、「ブロックハッシュ」と呼ばれるものでブロックを識別するためのIDです。
各ブロックは
...
を保持しています。これらの情報をまとめてハッシュ化することでブロックハッシュを計算します。
ただし、このブロックハッシュには正解とみなせるあるルールが決められていて、
その正解とみなせるブロックハッシュ値が得られるまでnonceと呼ばれる値をいじりながら何度もブロックハッシュを計算します。
この「正解とみなせるブロックハッシュ値が得られるnonce」を見つけることがゴールとなります。

正解とみなせるブロックハッシュとそれに対応するnonceを見つけるのにはそれなりの計算量が必要になります。
これについては「[マイニング](#マイニング)」で詳細を説明します。
努力の末、正解とみなせるブロックハッシュ値が得られるnonceを見つけたノードは、
「俺nonce発見しました！（発掘しました！）」と
ビットコインネットワーク全体に宣言します。
宣言をうけたノードたちは、このブロックの情報とnonceを受け取り、ハッシュを計算することで
正解とみなせるブロックハッシュ値となるか検証します。
その結果、正解とみなせるブロックハッシュとなった場合は「検証成功」ということで、
各ノードはそのブロックを信頼でいるものとして自身のブロックチェーンに追加します。

この「nonceを見つけるのにはそれなりの計算量が必要」な行為がProof of workと呼ばれる概念であり、
ビットコインを改竄不可能と言わしめている理由の一つと言えます。
なぜなら、過去の取引を改竄した場合、その過去の時点までのブロックハッシュを全て計算し直すことになり、
その行為が現実的な時間では不可能なためです。
結果として、ブロックチェーンに繋がっているブロック（の中のトランザクション）は
ビットコインネットワーク全体で信頼を得たデータとして未来永劫保持され続けることになります。
特に、ブロックが追加されれば追加されるほど改竄が難しくなるため、
より信頼できるデータとみなされます。

このように、ビットコインの世界では不正のない通常の取引を承認するだけでも相当量の計算処理を必要とします。
一見無駄に見えますが、この仕組こそが不正なデータを許容しないビットコインの根幹部分となっています。

膨大な計算の結果ビットコインの世界では、ブロックチェーンのデータこそが絶対なる正となります。
ビットコインネットワークに参加しているノード達は、このブロックチェーンを参照して、
誰がいくらのビットコインを持っているのかを判断します。つまり、残高のマスターデータと言えます。


## マイニング

「[ブロックチェーン](#ブロックチェーン)」で、ブロックチェーンを作るために、
各ノードは各ブロックのIDとなる「ブロックハッシュ」を計算する必要があると説明しました。
ここで、先程登場した「ブロックチェーンのデータ構造」の図にある各ブロックハッシュが
特殊な文字の並び（0が何個も続いている）になっていることが分かります。
実は、ブロックハッシュには「先頭に0がN個続かないといけない」というルールがあります。
このNの数は時期によって増加します。2018年7月現在は、0が18個続く必要があるようです。

```
// 実際のブロックハッシュ
000000000000000000301461586ee8cebf74f9b8fb8c06fac9a79c163ffd8021
```

この0が何個も続くブロックハッシュは、ブロック内のデータを「ハッシュ化した結果」ですが、
ハッシュ化した結果0が何個も続くような値が得られるとは限りません。ちょうど以下の図様な感じです。

---

![ブロックハッシュの計算](../resources/img/bitcoin_blockhash_calculation.svg)

*ブロックハッシュの計算*

---

このように、何も考えずにブロックハッシュを計算すると先頭に0がN個続くようなハッシュ値は得られません。
そこで「先頭に0がN個続かないといけない」というルールを満たすブロックハッシュを得るために、
「nonce」と呼ばれる変数を利用します。
このnonceという値は数値で表現されどんな値でも構いません。
むしろ、このnonceの値を調整して先頭に0がN個続くブロックハッシュとなった場合に、
そのnonceが正解の値として認められるようなイメージです。

つまり、上の図のように、nonceの値をひたすら変えながら
ブロックハッシュが「偶然0がN個連続するハッシュ値」になるまでひたすら計算を繰り返します。
これが俗にいう「マイニング」という作業になります。（詳細は「」





 - [Blockchain Data API - blockchain.info](https://www.blockchain.com/api/blockchain_api)
