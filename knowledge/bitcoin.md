...
#### ②　ビットコインのネットワークに参加している全てのノード（コンピューター）が共通の残高の認識を得ないといけない

これが俗に言う「ビザンチン将軍問題」の課題です。
ビットコインのネットワークに参加しているノード（PC）群がそれぞれ違う残高と認識していては取引が破綻していまいます。

 - 日本のノード   「Aさんは100万持ってる。Bさんに10万円送金！世界中のノードたちよ、この取引確認して！」
 - アメリカのノード「え、Aさんって5万しか持ってないんじゃないの？」
 - 日本のノード   「いやいやAさんは確かに100万だよ」
 - アメリカのノード「ぁあん？！」
...
「①　嘘の取引かどうか見極めないといけない」の問題の解決策について説明します。
そもそも何が難しいのか説明します。
まず、一般的な銀行を例にすると、銀行は中央管理型の仕組みなので

 - Aさん「Bさんに1万円送金したい！」
 - 中央管理サーバー「分かりました。まずはAさんか確認するのでカードとパスワードを入力してね」
 - Aさん「カードウィーン、ポチポチ、ほら、私です」
 - 中央管理サーバー「たしかに。じゃあ、Bさんに送っときます！」
 - Bさん「お、確かにAさんから1万振り込まれてる！」

という感じで、嘘の取引でないことを中央管理のサーバーが担保してくれます。
一方ビットコインはどうかというと中央管理サーバーがいないので

 - 隣のノード「AさんがBさんに1万送信したってよ。確認して」
 - 自分のノード「そ、そうなんだ。りょうかい（多分本当だよな・・・）」

って感じで周りに存在するノードたちから情報が伝達されてくるだけです。
...
誰がいくらのビットコインを持っているのか過去にさかのぼってチェックする事が可能です。

また、各トランザクションにはInputとなった人の電子署名（signature）がついている事がわかります。
この電子署名があることで、確実に本人が作ったトランザクションであることを証明できます。
...
「Inputになってるトランザクションってどれ？存在しなくない！？不正じゃない！？！？」
となり、その取引は破棄されてしまいます。
...
この場合は、実在するトランザクションから偽のトランザクションを作り出す必要がありますが、
偽のトランザクションでInputとなる人（ここではZさん）の電子署名を偽造できないため、
こういった偽のトランザクションも作ることはできません。
...
この結果、本人以外が勝手にビットコインを送金することができない仕組みになっています。

もう少し具体的にすると以下の図のようになります。
...
まず、前提条件として各ユーザーは
 ...
 - 公開鍵（公開される鍵のデータ。Lock（暗号化）にしか使えない）
 - 秘密鍵（公開されない鍵のデータ。絶対流出させちゃいけないやつ。Lock（暗号化）, Unlock（復号化）に使える）

をそれぞれ持っている状態です。
ここでは公開鍵でLockしたデータは、秘密鍵でしかUnlockできないというポイントのみ覚えてください。

上の図のうち、「Alice」を起点に「Bobに0.085 ₿ 支払う」というトランザクション（上の図の真ん中のトランザクション）を生成する流れを見ていきます。

まず、最初に重要なのがAliceが「Bobに支払うためのビットコイン」に相当するトランザクションのOutputを持っているか、というところです。
これは上の図でいう一番左のトランザクションにあたります。ここでは持っているのでクリアということになります。

次に、左のトランザクションのOutputを使う（次のトランザクションのInputにする）ために、
Aliceは自身のウォレットにある秘密鍵でトランザクションのアウトプットをUnlockします。
...
最初に相手のビットコインアドレス（公会議）を指定し、そのビットコインアドレスに対していくらのビットコインを送金するかを指定します。
