...
|トランザクション|ビットコインの取引のデータ構造。AさんがBさんに 1 ₿ 支払った、みたいな情報|
...
|ビットコイン|ブロックチェーンを利用したP2P電子通貨システム。あるいは、このネットワークの取引で利用される通貨の単位|
...
トランザクションのOutputは次のトランザクションのInputとして利用されます。
ちょうど、以下の図のような感じです。

---

![トランザクションのデータ構造](../resources/img/bitcoin_transaction_simple.svg)
...
この図は、
 ...
 1. ZackからAliceへ 0.95 ₿ 支払う
 1. AliceからBobへ 0.90 ₿ 支払う
 1. BobからCassandraへ 0.85 ₿ 支払う

というトランザクションのチェーンを表しています。
txidは「トランザクションID」と呼ばれるトランザクションを識別するためのIDです。
図に示すとおり、ビットコインを受け取った人（Outputのビットコイン）は、
次のトランザクションでそのビットコインを使う（Inputにする）事ができます。
また、トランザクション内のInputとOutputのビットコインの金額が一致しないことが分かります。
この差額は「トランザクションフィー」と呼ばれる送金手数料で、
トランザクションを生成するたびに一定額の手数料が発生します。
このためInput = Outputとはなりません（後述します。またトランザクションフィーの金額は例なので実際の手数料とは異なります）。

また、重要なポイントとして、各トランザクションは1つ前のトランザクションIDを保持しているという点です。
...
的な不正行為を防げそうな気がします。なぜなら、AさんがBさんに 10 ₿ 送金するためには、
10 ₿のInputを持つトランザクションを作る必要があり、
このInputは前のトランザクションのOutputを元にして作る必要があるため、
「？？？さんからAさんに 10 ₿ の支払いがあった」というトランザクションが必要になります
（＝もちろんそんなトランザクションはないので、結果作れません）。
