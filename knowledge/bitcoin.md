...
もう少し簡単に言うと
 ...
 - Aさんの 1 bitcoin をBさんに 0.5 bitcoin 支払います

という情報を1つの単位とするようなデータです。
この誰から誰にいくらの部分は「Input」「Output」と呼ばれています。

 - Aさんの 1 bitcoint を　→　`Input`
 - Bさんに 0.5 bitcoin 支払います　→　`Output`
...
txidは「トランザクションID」と呼ばれるトランザクションを識別するためのIDを表します。
図に示すとおり、bitcoinを受け取った人（Outputの人）は、
次のトランザクションでそのbitcoinを使う（Inputにする）事ができます。
また、トランザクション内のInputとOutputのビットコインの金額が一致しないことが分かります。
この差額は「トランザクションフィー」と呼ばれる送金手数料で、
トランザクションを生成するたびに一定額の手数料が発生します。
このためInput = Outputとはなりません（後述します）。
