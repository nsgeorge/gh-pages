...
ここで、各ブロックハッシュが特殊な文字の並び（0が何個も続いている）になっていることが分かります。
...
この0が何個も続くブロックハッシュは、ブロック内のデータを「ハッシュ化した結果」の値ですが、
...
何も考えずにブロックハッシュを計算すると先頭に0がN個続くようなハッシュ値は得られません。
そこで「先頭に0がN個続かないといけない」というルールを満たすブロックハッシュを得るために、
「nonce」と呼ばれるハッシュ化する際の変数を利用します。
...
続いて、この「ブロックチェーン」と「全てのノードが全ユーザーの残高についてどのように共通認識を得ているか」がどう関係してくるのか説明します。
ビットコインの世界では、ブロックチェーンのデータこそが絶対なる正となります。
